...
**[SUPERSEDED 2026-06-19 — OP-1 close; see state_2026-06-19_op1-close.md]:** The 2026-06-02 position above is superseded. Grouping is no longer self-invented AI clustering — SKUs are snapped to the **Shopify Standard Product Taxonomy**, a cross-merchant-validated vocabulary, so single-brand data no longer has to name or validate a fine taxonomy (Shopify already did). Consequences: (1) **Sub-category depth is NOT Phase-2-deferred** — tag at the deepest taxonomy level where qualified signals concur (merchant-assigned depth is free via the Shopify-assigned category; LLM depth self-limits via deepest-agreement), and fire at the deepest level AL-19 permits. (2) **New-category naming is not "deferred" — it is simply not done**; always map to existing taxonomy nodes. (3) The **only** genuine cross-client (Phase-2) item is improving LLM assignment *accuracy* at depth — an enhancement, not a blocker. The old "single-brand cannot validate a fine sub-category taxonomy" rationale was specific to the retired self-clustering approach. **Honesty caveat:** this unblocks deep *tagging*, not deep *alerting* — per-brand volume still gates firing depth via AL-19. The "clustering-quality gate / return-rate coherence" basis referenced above is also retired; the authoritative grouping + firing-depth definition is now GATE D1-G3 (`d1_validation_gates.md`) and the "CATEGORY GROUPING + FIRING-DEPTH GATE" section (`technical_architecture.md`).
